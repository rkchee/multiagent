...
parts = dask.delayed(pd.read_excel)('/training_medical.xlsx', sheet_name='Cardiology and Infectious Disea')[0:60]
data = dd.from_delayed(parts)
def dataset(batch_num, batch_size, data):
...
class realworldEnv(gym.Env):
    def __init__(self, env_config):
        ...
        for i in data.index:
            ...
            self.store_list.append(state)
        
        # self.store_list = db.from_sequence(self.store_list, npartitions=20)
        # self.store_list = list(self.store_list)

        self.store_list = dask.persist(*self.store_list)
        print(np.shape(self.store_list))
    ...
    def reset(self):
        ...
        # self.obs = dask.persist(self.store_list[self.eps_id])
...
if __name__ == "__main__":
    ...
    batch_num = 1
    data = dataset(batch_num, batch_size, data)
    trainer = ApexTrainer(
        env=realworldEnv,
        config=dict(
            **{
