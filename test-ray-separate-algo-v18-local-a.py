...
class realworldEnv(gym.Env):
    def __init__(self, env_config):
        ...
        # self.data = tf.data.experimental.make_csv_dataset('train_set.csv', batch_size=self.batch_size, select_columns=columns, label_name='Answer.image.label', num_epochs=1, ignore_errors=True)
        # self.data = self.data.map(create_matrix)
        # self.random_test=random.randrange(0, 1, 1)
    ...
    def reset(self):
        ...
        # # print(self.answer[0])
        # self.qnatext, self.answers = datagen(self.data)
        # store_list = []
        # for i in range(self.batch_size):
        #     seq = [self.qnatext[0][i].numpy().decode('utf-8'), self.qnatext[1][i].numpy().decode('utf-8'), self.qnatext[2][i].numpy().decode('utf-8'), self.qnatext[3][i].numpy().decode('utf-8'), self.qnatext[4][i].numpy().decode('utf-8'), self.qnatext[5][i].numpy().decode('utf-8'), self.qnatext[6][i].numpy().decode('utf-8')]
        #     obs = form_matrix(seq)
        #     store_list.append(obs)
        
        # self.answer = self.answers[self.eps_id].numpy().decode('utf-8')
        # print("before while loop", self.answer)
        ...
        # print(np.shape(store_list[self.eps_id]))
        # self.obs = store_list[self.eps_id]
        self.obs = np.zeros((128, 256, 10))
...
if __name__ == "__main__":
    ...
    #     obs = form_matrix(seq)
    #     store_list.append(obs)
    # store_list_copy = np.asarray(store_list)
    # answers_copy = np.asarray(answers)
