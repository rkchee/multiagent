...
import tensorflow as tf # import tf.compat.v1.data.Dataset as dt
...
# test(t['Electra_Tensors'], t['answers'])
...
class realworldEnv(gym.Env):
    def __init__(self, env_config):
        ...
        # f = h5py.File('myfile.hdf5')
        # self.d = f['r']
        # array = da.from_array(self.d)
        self.fdata = h5py.File('combinedxy.hdf5', 'r')
        self.d = self.fdata['/x']
        self.ys = self.fdata['/y']

        self.array = test(self.d, self.ys)[0]
        self.answer = test(self.d, self.ys)[1]
    ...
    def step(self, action):
        ...
        self.obs_next = test(self.d, self.ys)[0]
        self.answer = test(self.d, self.ys)[1]
        print('action: ', action, 'self.answer: ', self.answer)
        self.rew = 0
        self.done = False
