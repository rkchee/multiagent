...
class TrainingElement:
    def __init__(self):
        ...
        self.features, labels = next(iter(self.data))
        # print(list(self.features.values())[0].numpy())
        # print(list(self.features.values()))
        # self.batchElement()
        # self.qbatch, self.labbatch = self.datagen(self.data)
        # element, features = self.data.take(1)
        # for item in features[:, 0]:
        #     print(item)



    def encode(self, sent_input):
 
        input_ids = tf.constant(self.tokenizer.encode(tf.compat.as_str(sent_input.numpy()), max_length=128, pad_to_max_length=128))[None, :]        
    ...
    def create_matrix(self, samples, targets):
        ...
        for k in self.state_cube:
            for elem in samples[k]:
                elem_conv = tf.py_function(func= self.encode, inp= [elem], Tout=[tf.int64]) 
                # elem_conv.set_shape([None])
                print(elem_conv)
                matrix1.append(elem_conv)
    ...
    # @tf.function
    # def tok_lazy(self, inp_tensor):
    #     input_ids = tf.constant(self.tokenizer.encode(inp_tensor, max_length=128, pad_to_max_length=128))[None, :]
    #     outputs = self.model(input_ids)
    #     outputs = np.squeeze(outputs)
    #     return outputs
        ...
        # for k in self.state_cube:
        #     for row in samples:
                # input_ids = tf.constant(self.tokenizer.encode(row.numpy().decode('utf-8'), max_length=128, pad_to_max_length=128))[None, :]  # Batch size 1
                # outputs = self.model(input_ids)
                # outputs = np.squeeze(outputs)
    ...
    def __call__(self, flask_request):
        ...
        return {
            ...
            "test_features": list(self.features.values())
        }        
