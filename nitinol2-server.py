...
class realworldEnv(gym.Env):
   def __init__(self, env_config):
        import tensorflow as tf
        low = np.full((10, 128, 256), -1000)
        high = np.full((10, 128, 256), 1000)
        output_low = np.full((32768, ), -1000)
        output_high = np.full((32768, ), 1000)

        # self.action_space = Box(low=-1.0, high=2.0, shape=(1, ), dtype=np.float32)
        self.action_space = Box(low=output_low, high=output_high, shape=(32768, ), dtype=np.float32)
        self.observation_space = Box(low=low, high=high, shape=(10, 128, 256), dtype=np.float32)
